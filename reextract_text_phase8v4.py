"""
re-extract text embeddings only for phase8_v4.

舊 NPZ 的 mean/std（audio latent）不變，只重新 encode Qwen2-Audio caption
取 text_features / text_features_c。

用法：
    source ~/venvs/dac/bin/activate
    cd ~/MeanAudio
    python reextract_text_phase8v4.py [--batch_size 64] [--dry_run]
"""
...
NEW_NPZ_DIR   = Path("/home/kojiek/research/meanaudio_training/npz_phase8v4")
TRAIN_TSV     = Path("/mnt/HDD/kojiek/phase4_jamendo_data/phase8_v4_train.tsv")
...
def main():
    ...
    for start in tqdm(range(0, total, bs), desc="Re-encoding text (V4)"):
        ...
        for i, npz_fname in enumerate(batch_npz_files):
            ...
            if new_path.exists():
                skipped += 1
                continue
            old_data = np.load(OLD_NPZ_DIR / npz_fname)
            np.savez(
                new_path,
                mean=old_data["mean"],
                std=old_data["std"],
