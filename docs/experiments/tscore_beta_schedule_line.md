...
### 措辭註記（2026-09-15，對照上方「判讀限制」）

上一段的數字與收線決定正確（validator admissible，8 run 皆 40 點），但三句結論需收斂才能對外引用：

- 「論文 (ii) 在我們的 S1 不成立」→ 我們測的是 logit-normal 冪次傾斜 + max-of-2，不是 Beta(α,1)；只能寫「受論文啟發的改編在 2k/20k 探針中未觀察到正則化效果」。
- 「傾斜本身有害」→ 均勻 t 格點 MSE 對不傾斜的 arm 天然有利。λ1.0 的差距集中在 t=0.1/0.3（+0.035 / +0.008），中段 t=0.5–0.7 反而略好（−0.002～−0.008），是 t 分配取捨，不是整體變差；未測生成品質。
- 「逐列 PE-AV 分數無穩定資訊」→ R3 兩 seed 反號、2 seed 檢定力低，只能寫「未檢出」。
