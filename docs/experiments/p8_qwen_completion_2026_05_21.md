...
The important conclusion is that Qwen single-cap NoQ also collapses under the P8 recipe. This falsifies the earlier narrow hypothesis that the P9.5 failure was caused only by multi-cap random-pick supervision. In this tested setting, removing multi-cap did not recover prompt conditioning.
...
2. Qwen single-cap NoQ collapses to the same CLAP region as the Qwen multi-cap and other Qwen variants.
...
Therefore, within this tested P8/P9.5 recipe, multi-cap random-pick is not necessary for collapse. The safer paper wording is that the Qwen captioner regime / caption distribution can induce the same muted-text conditioning failure even in a single-cap NoQ setup.
