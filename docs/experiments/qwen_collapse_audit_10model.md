...
|---|---|---|---|---|---|
| **P8-Qwen / Qwen-Rnd-NoQ** | Caption source: Qwen single-cap random. Architecture identical to P8. | 0.0611 | 0.06–0.12 | "Collapse is unique to multi-cap training format" | ❌ Falsified — single-cap Qwen also collapses |
...
| **P9.5 V1 / Qwen-Multi-NoQ** | 5-task Qwen multi-cap (full task-framed diversity) | 0.0609 | 0.02–0.04 | "More caption diversity (multi-cap) fixes single-cap collapse" | ❌ Falsified — more diversity makes it worse |

**Finding**: None of the Qwen caption strategies (random/best-consensus, single/multi-cap, NoQ/+Q) escape collapse. The failure traces to intrinsic properties of the Qwen caption *distribution*, not to architectural choices or selection strategy.
...
Collapsed cluster (0.058–0.069) — 10 models
  EXP-C  Qwen+prefix                  ▌  0.0580
  EXP-A  LP-MC stripped               ▌  0.0608
  P9.5 V1  Qwen-Multi-NoQ             ▌  0.0609
...
All 10 non-LP-MC-original training configurations fall in a tight cluster at MC CLAP 0.058–0.069, regardless of caption selection strategy, Q conditioning, multi/single-cap, style transfer, or data mixing.
...
| P9 V1 (LP-MC multi-5) | 0.065 | 0.084 | +0.019 | collapsed |
| P8-Qwen / Qwen-Rnd-NoQ | 0.058 | 0.078 | +0.019 | collapsed |
| P7V1-Qwen / Qwen-Rnd-Q | 0.060 | 0.079 | +0.019 | collapsed |
| P9.5 V1 / Qwen-Multi-NoQ | 0.060 | 0.080 | +0.020 | collapsed |
...
The +0.02 Qwen-prompt boost is a metric artifact (Qwen prompts are slightly easier to score high CLAP under HTSAT-base), **not** a sign of conditioning quality. Its universality across all collapsed models — LP-MC multi-cap, Qwen single/multi-cap, boilerplate-prepended — confirms a **shared failure mechanism**, not caption-source-specific failure.
...
| "Collapse is caused by multi-cap random-pick supervision" | ❌ Qwen single-cap (P8-Qwen, 0.0611) and multi-cap LP-MC (P9 V1, 0.065) both collapse |
...
| "Collapse is due to a pipeline bug" | ❌ 5 independent checks (NPZ CLAP/T5 cache, TSV alignment, T5 truncation, S2 loss convergence) all clean |
...
*Experiments completed 2026-05-09 through 2026-05-19. All 10 models use Jamendo 251K audio, 2-stage training (S1 400K FluxAudio + S2 200K MeanFlow), evaluated on MusicCaps n=5521.*
