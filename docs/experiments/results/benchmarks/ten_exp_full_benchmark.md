...
**與「CLAP-filter 訓練 + CLAP eval」資料洩漏不同**：P8V3 不是用 CLAP 閾值過濾訓練資料，而是把 CLAP sim 轉成 ordinal q 信號。失敗根因是「Q 信號與 genre 相關 → q_embed 學到 genre shortcut」，非「訓練-評測同指標」。詳見 `docs/experiments/history/phase4-phase8/Phase4_to_Phase8_Complete_Summary.md` 第十一節。
...
- Seed=42 TSV 生成：見 `~/MeanAudio/docs/experiments/results/benchmarks/ten_exp_full_benchmark.md` 中的 Python snippet（已直接寫死 random.seed(42)，1816 unique tracks）
...
5. P8V3 的 genre-shortcut 結論來自 piano 分箱實測（q=0~1: 10.4% piano, q=8~9: 21.2%，見 `docs/experiments/history/phase4-phase8/Phase4_to_Phase8_Complete_Summary.md`）。本表只呈現 benchmark 結果，機制分析在該 doc。
