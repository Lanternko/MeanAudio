...
- Both arms use the same 251,599 audio windows and the same upstream
  track-level Qwen captions (legacy filenames call these `official`; they are
  upstream Qwen outputs, not human-authored official labels), Qwen text
...
3. checks every Qwen row against the upstream Qwen JSON by parsed Jamendo
   track; this verifies track identity, not `segment_N`/10s temporal alignment;
