...
>
> ⛔ **2026-07-16 correctness correction**：P9.5 V1 multi-cap NPZ 由舊
> positional writer 產生，Qwen captions 與 canonical audio statistics 全量
> 錯配；0.0609、0.0594 與 steering 只保留為 corrupted-data artifacts。
> 本檔 §3–5 的 training-result interpretation 全部失效；caption JSONL corpus
> 本身仍有效。有效 Qwen single-cap 結果請見 `docs/experiments/history/phase8/qwen_rerun_summary.md`。
>
> ⚠️ **2026-05-08 update 已被 2026-07-16 correction 取代**：有效的 Qwen
> single-cap controls 仍呈低 prompt conditioning，但失效的 P9.5 multi-cap
> checkpoint 不能再用於 caption-regime vs multi-cap attribution。
...
| P9 V1 (NoQ, LP multi) | ~~0.0650~~ | 0.147 | **INVALID cache** |
| **P9.5 V1 (NoQ, Qwen multi)** | ~~**0.0609**~~ | **0.044** | **INVALID cache** |
...
→ **V2 當時依 gate SKIP**。但 gate 的 P9/P9.5 CLAP 依據已失效，不能再寫成「只會確認失敗」。

---

## 5. 解讀（嚴格分層 — Codex 5/5 review 收斂版）

### 5.1 歷史 artifact（2026-07-16 起不可作 observation-level evidence）

- ~~P9.5 V1 落在 collapse 區：MusicCaps 0.0609 / Jamendo 0.0594~~ → **失效 cache artifact**
- P9.5 V1 same-seed prompt steering ratio 全 4 pair 都很低：**0.022–0.044**
- P9/P9.5 的低 CLAP/steering 共同受同一 mapping corruption 污染，不能形成跨-captioner replication

### 5.2 撤回的舊推論

- ~~Collapse 不是 LP-MusicCaps seed-sampling 特有~~
- ~~Qwen task-framing diversity 沒救回 multi-cap random-pick 訓練~~
- ~~問題更可能來自 multi-cap supervision 形式~~
- clean canonical multi-cap rerun 前，這些問題全部維持 unknown
...
> Codex 5/5 推薦的「Qwen static single-cap NoQ」isolated control 已執行。完整 P8/P7V1/P4V2-Qwen 三組對照見 `docs/experiments/history/phase8/qwen_rerun_summary.md`，這裡只記對 §5 的修正。
...
| P9.5 V1 multi NoQ | **INVALID cache** | ~~0.0609~~ | 0.044 | — |

### 對 §5.2 推論的修正

舊推論「問題**更可能**來自 multi-cap supervision 形式」**不再成立**：

- ~~Qwen single-cap ≈ Qwen multi-cap~~ → **撤回**；P9.5 multi-cap pairing 失效
...
- multi-cap 在 Qwen 或 LP-MC regime 的效果目前均 unknown
...
- 真正能說的只剩：有效的 Qwen single-cap controls 在目前配方下呈低 prompt conditioning；multi-cap trigger claim 撤回

### 仍不能宣稱

- ❌ 對 multi-cap 作正面或負面 effect claim（舊 LP/Qwen multi results 均失效）
...
> Valid single-cap controls show low prompt conditioning for the tested Qwen
> caption regime. The P9/P9.5 multi-cap runs are excluded because their
> audio-caption caches were misaligned; no multi-cap conclusion is retained.
...
2. **鎖定主線 narrative**：
   ...
   - ~~Phase 9 / 9.5：multi-cap random-pick 失敗~~ → **撤回，cache misaligned**
   - P8 V4：把數字寫進 prompt 失敗 → 支持「embedding condition」優於「prompt token」
3. **補最小圖表**（不補大實驗）：
   - P9/P9.5 steering plot 不得再用來支撐 multi-cap 主張
...
> The historical P9.5 result is excluded because the multi-caption cache paired
> captions with the wrong audio statistics. It provides no evidence about
> multi-cap random-pick training; a canonical clean rerun is required.
