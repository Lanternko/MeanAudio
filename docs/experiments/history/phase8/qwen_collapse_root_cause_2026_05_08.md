...
Valid MeanAudio single-cap controls trained on **Qwen2.5-Omni-3B captions**
reach MusicCaps CLAP ≈ 0.06 with steering ratio < 0.12 across the tested
selection and Q/NoQ configurations. LP-MC single-cap counterparts reach CLAP
0.18–0.20 with steering 0.9–1.7. This document audits 5 hypotheses (H1–H4 +
H7/H9) using cheap diagnostics (no new training).

> ⛔ **2026-07-16 correction**：P9 V1/V2 and P9.5 V1 multi-cap caches
> paired captions with the wrong audio statistics because the old writer used
> positional `i.npz` lookup. All multi-cap rows and any shared-mechanism,
> single-vs-multi, or P9 residual claims in this historical analysis are
> excluded. Valid Qwen single-cap and other independently generated caches are
...
| P9 V1 | **INVALID multi cache** | NoQ | ~~0.0650~~ | ~~0.0650~~ | 0.147 |
| P9 V2 | **INVALID multi cache** | +Q | ~~0.0403~~ | ~~0.0403~~ | 0.056 |
| **P8-Qwen** | **Qwen single** | NoQ | **0.0611** | 0.0582 | **0.120** |
| **P7V1-Qwen** | **Qwen single** | +Q | **0.0687** | 0.0598 | **0.057** |
| **P9.5 V1** | **INVALID multi cache** | NoQ | ~~**0.0609**~~ | ~~0.0597~~ | **0.044** |
| **P4V2-Qwen** | **Qwen single (BC-selected)** | NoQ | **0.0611** | **0.0596** | TBD |

P8-Qwen is low despite being single-cap → the valid conclusion is limited to
the tested Qwen regime: **Qwen single-cap training is sufficient to reproduce
the low-conditioning behavior under this recipe**. This does not estimate the
effect of multi-cap training.
...
| P9V1 (LP-MC NoQ multi-5) | INVALID cache | ~~0.0650~~ | ~~0.0837~~ | ~~+0.019~~ | excluded |
| P9V2 (LP-MC +Q multi-5) | INVALID cache | ~~0.0403~~ | ~~0.0622~~ | ~~+0.022~~ | excluded |
...
| P9.5 V1 (Qwen NoQ multi-5) | INVALID cache | ~~0.0597~~ | ~~0.0799~~ | ~~+0.020~~ | excluded |
...
**G1+G2 corrected 2026-07-16:** several valid low-CLAP Qwen single-cap
checkpoints show a +0.019–0.020 Qwen-prompt boost, while healthy P8 has +0.084.
The former claim of universality across multi-cap LP-MC and all Qwen variants
is withdrawn because the multi-cap rows are invalid.

**Valid H1 evidence:** LP-MC trained healthy model + Qwen prompts → 0.2246,
showing that Qwen prompts carry usable conditioning information at inference.
The valid Qwen single-cap models remain low under both prompt sources. The
former multi-cap LP-MC comparison is excluded.

**2026-07-16 correction to former G1 finding:** the P9V1 +0.019 row comes
from a misaligned cache, so its numerical similarity to P8-Qwen cannot support
a shared failure mechanism. The valid Qwen single-cap signature remains a
behavioral result scoped to that caption regime.
...
| LP-MC multi-cap (P9V1) | **INVALID cache** | excluded |

→ The valid rows motivate a caption-regime hypothesis; the excluded P9 row no
longer supports a structural-instability or multi-cap generalization.
...
**G2 corrected 2026-07-16:** the +0.019–0.022 boost remains in the valid
Qwen single-cap/selection and LP-perturbed rows. The former extension to
multi-cap LP-MC/Qwen and the multi-vs-single boundary is withdrawn.
...
**G3 corrected 2026-07-16:** the cluster remains visible across several valid
Qwen/LP-perturbed configurations, but excluded multi-cap rows no longer support
a universal separator or multi-cap mechanism.
