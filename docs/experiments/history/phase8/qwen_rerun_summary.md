...
>
> **2026-07-16 correction**：P9 V1/V2 與 P9.5 V1 multi-cap results
> 使用同一個 positional cache-writer bug，audio–caption pairing 全量錯配。
> 本文件的 Qwen single-cap / BC / Q controls 仍有效；所有引用 LP/Qwen
> multi-cap 數字的比較與 paper narrative 已撤回。
...
| LP-Multi-NoQ (P9 V1) | ~~0.0650~~ INVALID | — | — | — | — |
| LP-Multi-Q (P9 V2) q=9 | ~~0.0403~~ INVALID | — | — | — | — |
| **Qwen-Multi-NoQ** (P9.5 V1) | ~~0.0609~~ **INVALID** | 6.07 | 6.63 | 5.42 | 6.52 |
...
| Qwen-Multi-NoQ (JMQ) | ~~0.059~~ | ~~0.080~~ | ~~+0.086~~ | ~~0.54%~~ (invalid checkpoint) |
...
LP-Multi-NoQ (P9 V1):         0.025-0.147 ← invalid cache artifact
LP-Multi-Q (P9 V2 q=8):       0.012-0.056 ← invalid cache artifact
Qwen-Multi-NoQ (P9.5 V1):     0.022-0.044 ← invalid cache artifact
...
1. **MC eval（預設）**：3 個有效 Qwen single-cap/selection/Q controls 的 MC CLAP 0.061-0.069，遠低於 LP-trained 0.185-0.198
2. **JMQ eval（in-distribution Qwen prompts）**：有效 Qwen-Rnd-NoQ / Qwen-Rnd-Q 約 0.078-0.079；reverse control LP-Rnd-NoQ (JMQ) **0.225** healthy
3. **Steering ratio**：有效 Qwen single-cap controls max < 0.15
4. **PE-AV peav_score**：有效 Qwen single-cap controls 在 JMQ 約 +0.083–0.085；LP reverse on JMQ +0.193
5. **t2a R@10**：有效 Qwen single-cap controls 0.20–0.49%；LP reverse on JMQ **10.3%**
...
3. **Q signal 在 Qwen single-cap regime 沒有恢復 healthy range**：
   - Qwen-Rnd-Q (0.069) vs Qwen-Rnd-NoQ (0.061) — 仍遠低於 LP healthy range
4. **P9.5 V1 不再參與 attribution**：其 multi-cap cache 錯配，不能比較 multi vs single

### 3.3 不能宣稱

- ❌ 對 LP 或 Qwen regime 宣稱任何 multi-cap effect — P9/P9.5 multi-cap checkpoints 均失效
...
## 4. Paper narrative 主軸（2026-07-16 更正版）

> In the valid single-caption controls, replacing LP-MC supervision with the
> tested Qwen2.5-Omni caption regime is associated with muted prompt
> conditioning (LP-Rnd-NoQ 0.185 vs Qwen-Rnd-NoQ 0.061).
>
...
> — the prompt format alone does not explain the valid Qwen single-cap results.
> No claim about multi-cap supervision is retained until the canonical clean
> all-five-caption rerun is complete.
...
- 設計討論：`docs/experiments/history/phase9/phase9_design.md`
- P9.5 V1 結果：`docs/experiments/history/phase9/phase9_5_summary.md`（pre-Qwen-rerun，narrative 已被 5/7 update 修正）
...
- `segments_no_vocals/.../segment_N.mp3` 是約 30.0408s 的 local segment；目前 251,599-row
  訓練 manifest 全部只保留 suffix `_0`，實際窗口為該 segment 的**前 10s**
  (`start_sample=0, end_sample=160000` at 16kHz)。
- NPZ 存 312 frames = 9.975s audio latent
- **LP-MC segment caption** 的鍵包含 `segment_N`，但描述約 30s local segment，故為
  30s→10s mismatch；同 track 的不同 segments 並非共用同一筆 LP-MC caption。
- 歷史檔名中的 **official Qwen** 是上游 reference repo 提供的 Qwen 產物，不是人工
  官方標註。其 JSON 每個 track 一筆，舊 mapper 只用 track ID，因而把一筆 caption
  廣播到同 track 的所有 retained segments，屬更嚴重的 track→10s mismatch。

兩條 pipeline 都有 audio-window mismatch，但程度不同，不能再簡寫為「LP-MC + Qwen
都是同一種 30s caption bug」。LP-MC 為什麼在 segment→10s mismatch 下仍 healthy，
而 upstream track-level Qwen collapse，仍可能涉及時間對齊、captioner style、描述廣度
與 training dynamics；現有比較無法分離這些因素。

**修正後的 causal test**：不應只做 30s audio context retrain，因為那會同時改變
latent sequence length 與模型計算量。乾淨設計是在相同 10s latents 上使用相同 caption
corpus，僅做同 track 內 segment-caption 循環錯配；另以 matched 10s concise/rich captions
測 style/breadth。完整 2×2 設計見上述權威文件。
