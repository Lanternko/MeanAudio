...
| **Autoguidance 線**（080、負向分支換成同一 run 較早的 EMA 快照、吃同一條文字；S2 110k/130k 為主、S1 快照探索；pure 對 stock cfg0、combo 對 stock cfg3_neg，前 N 列與 stock 逐樣本配對；主端點 lvl30 dPQ vs 同批 G_neg） | `docs/experiments/autoguidance_ema_snapshot_20260926.md` |
| **Autoguidance 結果**（080 收線：S2 快照當 bad 沒幫助、高 w 是響度效應；只有探索用 S1 30k 有穩定 +0.061 lvl30 PQ（全量，= 文字負向增益 7%）且 PC −0.24；combo 3 seed 都比 CFG3+neg 差 0.05–0.07 → 不可疊加；同文字較差模型給不出 fidelity8 的效果，負向槽的**文字**仍是必要的） | `docs/experiments/results/autoguidance_ema_snapshot_20260926_results.md` |
| **真品質標籤前綴重訓線**（081–083、QA-MDT 式：語料自身 AES PQ 最差／最好 20% 的 caption 加 `Low/High quality recording.` 前綴，quarter × 3 seed，對讀 nmv2pair control；主端點：短負向 prompt `Low quality recording.` 的 lvl30 PQ 增益差中差 ≥ 0.19；標籤與端點同為 PQ → CLAP 是唯一獨立讀數） | `docs/experiments/quality_label_prefix_081_20260926.md` |
