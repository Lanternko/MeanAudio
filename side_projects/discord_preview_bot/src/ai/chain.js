...
async function generateAIReply(message, userText) {
  ...
  // Group context is appended for THIS call only — it never lands in conv
  // memory, so each @ gets a fresh window of what the group is actually
  // talking about. Without it 西寶 cannot interpret stickers or cross-talk
  // between people other than her. Volatile → appended LAST so it doesn't
  // bust the cache on the stable prefix above.
