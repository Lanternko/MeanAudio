#!/usr/bin/env python3
"""MeanSim K=3 from slot0 + slot1 + slot3 (same C2.0 prompt; T/seed only)."""
...
SLOT1 = FULL / "slot1_temp115.jsonl"
SLOT3 = FULL / "slot3_temp100.jsonl"
...
def main() -> None:
    ...
    missing = [i for i in ids if not (i in s0 and i in s1 and i in s3)]
    if missing:
        raise SystemExit(f"incomplete 0/1/3 coverage: missing {len(missing)} / {len(ids)}")
    print(f"encoding n={len(ids)} slots=0,1,3", flush=True)
    ...
    e3 = model.encode([s3[i] for i in ids], convert_to_tensor=True, normalize_embeddings=True, batch_size=256)
    s01 = (e0 * e1).sum(dim=-1)
    s03 = (e0 * e3).sum(dim=-1)
    s13 = (e1 * e3).sum(dim=-1)
    scores = ((s01 + s03 + s13) / 3.0).detach().cpu().numpy()
    ...
    for bucket in range(k):
        ...
        for idx in ranked[cursor: cursor + size]:
            q[idx] = CODES[bucket]
        cursor += size
    out_tsv = FULL / "phase8_caption2p0_fair013_k3_balanced_train.tsv"
    ...
    (FULL / "FAIR013_K3_SUMMARY.json").write_text(json.dumps(summary, indent=2) + "\n")
