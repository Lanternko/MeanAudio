...
#   CFG 4.5 is historical guided-eval only. Labels containing cfg4p5 are
#   rewritten to cfg0 so a live caller cannot skip-reuse 4.5 metrics.
#
# Usage:
...
if [[ "$LABEL" == *cfg4p5* ]]; then
  NEW_LABEL="${LABEL//cfg4p5/cfg0}"
  echo "RELABEL_CFG4P5_TO_CFG0 $LABEL -> $NEW_LABEL"
  LABEL="$NEW_LABEL"
fi
...
echo "EVAL_MF25 $LABEL cfg=0 $(date -u +%FT%TZ)"
if [ ! -f "$METRICS" ]; then
  mkdir -p "$OUT/audio"
...
payload = {
    ...
    "fair_compare_anchor": "caption2p0_s2_mf25_cfg0 CLAP=0.2157 (full S2; recorded as CFG 0.5, equivalent for cfg<1)",
    "cfg_strength": 0,
    "checkpoint": str(ckpt),
    "metrics": vals,
