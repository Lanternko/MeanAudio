...
用法：
  ...
  tmux new-session -d -s qwen_omni \\
    "cd ~/research/meanaudio_training && \\
     ...
     python gen_qwen25omni_captions.py --slot all 2>&1 | tee ~/logs/qwen_test_seed42_2048_captions.log"
...
輸出：
  /mnt/HDD/kojiek/phase4_jamendo_data/qwen_test_seed42_2048_captions_slot{N}.jsonl  （每個 slot）
  /mnt/HDD/kojiek/phase4_jamendo_data/qwen_test_seed42_2048_captions.jsonl          （merge 後）
...
DEFAULT_INPUT_TSV = Path('/mnt/HDD/kojiek/phase4_jamendo_data/phase4_test_seed42_2048.tsv')
AUDIO_ROOT   = Path('/home/hsiehyian/dataset/segments_no_vocals')
OUTPUT_DIR   = Path('/mnt/HDD/kojiek/phase4_jamendo_data')
MODEL_ID       = 'Qwen/Qwen2.5-Omni-3B'
DEFAULT_BATCH_SIZE = 20    # 降至 20（~20GB）以讓出空間給共用 GPU 上 hsiehyian 訓練（7.7GB）；原 32→28GB 會 OOM
...
def run_slot(
    ...
    resume: bool,
    limit: int = None,
    input_tsv: Path = DEFAULT_INPUT_TSV,
    output_prefix: str = 'qwen_test_seed42_2048_captions',
    batch_size: int = DEFAULT_BATCH_SIZE,
):
    assert 0 <= slot <= 4, f'slot 必須是 0–4，得到 {slot}'
    prompt = PROMPTS[slot]
    suffix = f'_sanity{limit}' if limit else ''
    out_path = OUTPUT_DIR / f'{output_prefix}_slot{slot}{suffix}.jsonl'

    print(f'\n=== Slot {slot} ===')
    print(f'Prompt: {prompt}')
    print(f'輸入: {input_tsv}')
    print(f'輸出: {out_path}')
...
def merge_slots(
    ...
    slot_maps = []
    for slot in range(5):
        path = OUTPUT_DIR / f'{output_prefix}_slot{slot}.jsonl'
        if not path.exists():
    ...
    out_path = OUTPUT_DIR / f'{output_prefix}.jsonl'
...
def parse_args():
    ...
    p.add_argument('--input_tsv', type=Path, default=DEFAULT_INPUT_TSV,
                   help='輸入 TSV，需包含 id 欄位')
    p.add_argument('--output_prefix', default='qwen_test_seed42_2048_captions',
                   help='輸出檔名前綴；會產生 {prefix}_slotN.jsonl 與 {prefix}.jsonl')
    p.add_argument('--batch_size', type=int, default=DEFAULT_BATCH_SIZE,
                   help=f'Qwen batch size，default={DEFAULT_BATCH_SIZE}')
    return p.parse_args()
...
if __name__ == '__main__':
    ...
    if args.slot == 'merge':
        merge_slots(input_tsv=args.input_tsv, output_prefix=args.output_prefix)
    elif args.slot == 'all':
        for s in range(5):
            run_slot(
                s,
