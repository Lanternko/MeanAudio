#!/bin/bash
# ============================================================
# P4V2-Qwen: JamendoFull-QwenOmni-BestConsensus-NoQ (single-cap)
# train_pipeline_p4v2_qwen.sh
#
# Captioner-only control for P4 V2 (LP-MC best-consensus NoQ, MC CLAP 0.1909):
#   - Same training recipe as historical P4 V2 except caption source
#   - 5 Qwen task-framed captions per clip → BestConsensus pick
#     (highest pairwise mean_sim row = most consistent with other 4 caps)
#   - multi_cap=False (single fixed cap)
#   - NoQ
#
# Compare to P8-Qwen (random) to see if BestConsensus selection matters
# WITHIN Qwen, on top of comparing Qwen vs LP-MC.
#
# Pre-flight assumes:
#   - qwen_singlecap_bc_train.tsv generated
#   - phase9_5_bc_singlecap_npz/ generated (slice --variant bestconsensus)
#
# Usage:
#   tmux new -s p4v2_qwen
#   bash train_pipeline_p4v2_qwen.sh
# ============================================================
...
NPZ_DIR="$HOME/phase9_5_bc_singlecap_npz"   # BestConsensus single-cap
EVAL_SCRIPT="$HOME/research/meanaudio_eval/phase4_eval.py"
PEAV_SCRIPT="$HOME/research/meanaudio_eval/peav_eval.py"

TRAIN_TSV="$DATA_DIR/qwen_singlecap_bc_train.tsv"
...
echo "======================================================"
echo "  P4V2-Qwen — JamendoFull-QwenOmni-BestConsensus-NoQ (single-cap)"
...
# ============================================================
echo "[Pre-flight A] verify 3 bug fixes"
...
for i in samples:
    ...
    if tf.shape != (77, 1024) or tfc.shape != (512,):
        bad.append((i, tf.shape, tfc.shape))
if bad:
    print(f'[FAIL] {len(bad)}/200 sampled NPZs have wrong shape (expect (77,1024)/(512,))')
    for i, t1, t2 in bad[:5]:
        print(f'  idx {i}: tf={t1} tfc={t2}')
    sys.exit(1)
print(f'[OK] 200/200 sampled NPZs are single-cap (77,1024)/(512,)')
PYEOF
...
echo "    P4 V2 LP-MC BestConsensus NoQ MC CLAP: ~0.1909 (Jamendo subset)"
echo "    P8 LP-MC Random NoQ MC CLAP          : 0.1851"
echo "    P8-Qwen Random NoQ MC CLAP           : (this rerun's reference)"
echo ""
echo "  Question: does picking the most-consistent Qwen caption (BestConsensus)"
echo "  beat random selection within Qwen single-cap regime?"
echo "======================================================"
