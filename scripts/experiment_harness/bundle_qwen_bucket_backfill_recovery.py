...
R=Path('/home/kojiek/MeanAudio'); C=R/'docs/experiments/qwen_bucket_quarter_eval_backfill_recovery_contract.json'
...
def main():
    ...
    contract={'document_kind':'experiment_contract','schema_version':'1.0.0','schema_bundle_id':'harn-schema-v1','experiment_id':eid,'run_id':rid,
        ...
        'phases':[{'phase_id':'qwen-bucket-14-cell-backfill','action_id':'launch','input_artifacts':artifacts,'output_paths':[x['report'] for x in c['cells']]+[c['summary']],'completion_evidence':[{'path':c['bindings']['action'],'sha256':sha(c['bindings']['action'])}],'resume_action_id':'resume'}],
        'filesystems':[{'path':c['storage']['path'],'hard_floor_bytes':c['storage']['hard_stop_free_bytes'],'warning_floor_bytes':c['storage']['warning_free_bytes'],'peak_additional_bytes':15400000000,'transient_bytes':25000000,'recovery_reserve_bytes':53687091200}],
    ...
    preflight={'document_kind':'preflight_report','schema_version':'1.0.0','schema_bundle_id':'harn-schema-v1','experiment_id':eid,'run_id':rid,'contract_raw_sha256':ch,
        'approval_evidence':{'evidence_id':'approval-20260914-qwen-recovery','source_kind':'trusted_operator_record','trusted_channel':'operator_console','channel_record_id':'codex-task-20260914-qwen-recovery','channel_record_sha256':sha(c['approval_record']),'approver_id':'responsible-operator','issued_at':now,'expires_at':until,'experiment_id':eid,'run_id':rid,'bindings':{'contract_raw_sha256':ch,'policy_bundle_sha256':policy,'schema_bundle_sha256':schema,'runtime_sha256':runtime,'repair_envelope_sha256':None,'command_set_sha256':commandhash}},
        'checks':checks,'storage':[{'path':c['storage']['path'],'measured_at':now,'free_bytes':free,'hard_floor_bytes':53687091200,'peak_additional_bytes':15400000000,'transient_bytes':25000000,'recovery_reserve_bytes':53687091200,'verdict':'pass'}],'derived_verdict':'pass','created_at':now}
