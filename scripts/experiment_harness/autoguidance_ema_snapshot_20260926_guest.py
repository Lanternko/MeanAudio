#!/usr/bin/env python3
"""080 P2 guest: eval-only supervisor for the autoguidance EMA-snapshot experiment.
...
def progress(c):
    ...
    # Progress is scored cells plus clips written so far; the per-clip count keeps a
    # healthy generation (up to ~40 min for a 5521-row cell) off the stall threshold.
    cells = len(list((root / 'cells').glob('*.json'))) if (root / 'cells').is_dir() else 0
    clips = len(list((root / 'cells').glob('*/audio/*.flac'))) if (root / 'cells').is_dir() else 0
    ...
    try:
        while True:
            ...
            if request:
                ...
                atomic_json(resume, {'document_kind': 'autoguidance_ema_snapshot_resume_v1',
