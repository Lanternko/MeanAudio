#!/usr/bin/env python3
"""Durable HARN controller for Caption 2.0 full-scale K=3 S2Q."""
...
STATE = Path("/home/kojiek/logs/caption2p0_k3_s2q_harn")
PYTHON = Path("/home/kojiek/venvs/dac/bin/python")
PREREG = ROOT / "docs/experiments/caption2p0_full_k3_s2q_contract.json"
...
ACTION_RUNNER = ROOT / "scripts/training_pipelines/run_caption2p0_k3_s2q_action.sh"
TSV = ROOT / "data/phase8_caption2p0_k3_balanced_train.tsv"
ASSIGNMENT = Path("/mnt/HDD/kojiek/phase4_jamendo_data/phase8_qwen_meansim_k3_balanced.tsv")
...
AUDIT = Path("/home/kojiek/logs/phase8_qwen_caption2p0_s2q_from_noq_full_k3_balanced_BINDING_AUDIT.json")
DIAGNOSTIC = Path("/home/kojiek/logs/phase8_qwen_caption2p0_s2q_from_noq_full_k3_balanced_FINAL_METRICS.json")
FAIR = Path("/home/kojiek/logs/phase8_qwen_caption2p0_s2q_from_noq_full_k3_balanced_stage2_200000_musiccaps_n5521_mf25_cfg4p5_q9_REPORT.json")
CHECKPOINT = ROOT / "exps/phase8_qwen_caption2p0_s2q_from_noq_full_k3_balanced_stage2_200000/phase8_qwen_caption2p0_s2q_from_noq_full_k3_balanced_stage2_200000_ema_final.pth"
...
def configure() -> None:
    ...
    harn.RUNNER = TRAINER
    harn.EXPERIMENT = "phase8-caption2p0-full-k3-s2q"
    harn.RUN_ID = "run-20260815-caption2p0-full-k3-s2q"
    harn.KS = (3,)
...
def validate_audit() -> None:
    ...
    if payload.get("status") != "passed" or payload.get("rows_checked") != 251599:
        raise RuntimeError("Caption 2.0 K=3 binding audit did not pass")
...
def validate_diagnostic() -> None:
    ...
    if payload.get("status") != "passed" or payload.get("k") != 3 or payload.get("strategy") != "balanced":
        raise RuntimeError("Caption 2.0 K=3 diagnostic report did not pass")
    if payload.get("model", {}).get("sha256") != harn.digest_file(CHECKPOINT):
        raise RuntimeError("Caption 2.0 K=3 checkpoint binding mismatch")
...
def validate_fair() -> None:
    ...
    if payload.get("status") != "passed" or set(payload.get("metrics", {})) != required:
        raise RuntimeError("Caption 2.0 K=3 fair report is incomplete")
    if payload.get("provenance", {}).get("checkpoint_sha256") != harn.digest_file(CHECKPOINT):
        raise RuntimeError("Caption 2.0 K=3 fair report checkpoint mismatch")
    ...
    while preflight["derived_verdict"] != "pass":
        ...
        if preflight["derived_verdict"] == "pass":
            ...
            harn.atomic_json(harn.PENDING_QUEUE, {
                "schema_version": 1, "status": "promoted_to_harn", "order": [3],
...
def run() -> None:
    ...
    try:
        ...
        harn.notify("start", "Started Caption 2.0 full-scale K=3 S2Q chain: audit, training, diagnostic, and MF25 fair eval.")
        ...
        validate_audit()
        gate(ledger, contract, preflight, "binding_audit", "Caption 2.0 K=3 fresh binding audit passed; promoting training.", AUDIT)
        ...
        if not DIAGNOSTIC.is_file():
            run_action(commands["train_fresh"], "K=3 training and diagnostic evaluation")
        validate_diagnostic()
        gate(ledger, contract, preflight, "train_and_diagnostic", "Caption 2.0 K=3 training and diagnostic evaluation passed; promoting fair eval.", DIAGNOSTIC)
        if not FAIR.is_file():
            run_action(commands["fair_eval"], "K=3 MF25 fair evaluation")
        validate_fair()
        event = harn.append_event(ledger, "gate_result", verdict="pass", phase="fair_eval", notification="pending")
        harn.notify("fair_eval_complete", "Caption 2.0 full-scale K=3 S2Q MF25 fair evaluation completed.", report=FAIR)
        harn.append_event(ledger, "notification_delivery", relation=event, phase="fair_eval", notification="delivered")
        harn.write_generation(contract, preflight, ledger, "active")
        harn.terminal(contract, preflight, ledger, True, "Caption 2.0 full-scale K=3 S2Q chain completed.")
