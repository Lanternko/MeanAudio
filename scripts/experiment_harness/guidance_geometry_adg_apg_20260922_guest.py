#!/usr/bin/env python3
"""073 P2 guest: eval-only supervisor for the guidance-geometry (ADG/APG) experiment.
...
def progress(c):
    ...
    # Progress is scored cells plus clips written so far. Cells are the coarse
    # signal; the per-clip count is what keeps a healthy 30-minute generation off
    # the stall threshold, the same reason the 072 guest counts plan files.
    cells = len(list((root / 'cells').glob('*.json'))) if (root / 'cells').is_dir() else 0
    clips = len(list((root / '_audio').glob('*/*.flac'))) if (root / '_audio').is_dir() else 0
    return (cells * 100000 + clips,
    ...
    try:
        while True:
            ...
            if request:
                ...
                atomic_json(resume, {'document_kind': 'guidance_geometry_resume_v1',
