#!/usr/bin/env python3
"""072 P2 guest: eval-only supervisor for the shallow downward-limiting ladder.
...
def progress(c):
    ...
    # Two differences from the 063 guest, both of which would otherwise hold a
    # healthy job at the stall threshold:
    #   items are flat here (items/<id>.json, not items/<arm>/<id>.json);
    #   the CPU planning phase (pre-gain bisection per clip per limited arm) writes
    #   only plan/<id>.json and can run for tens of minutes before the first item.
    return (len(list((root / 'plan').glob('*.json'))) + len(list((root / 'items').glob('*.json'))),
    ...
    try:
        while True:
            ...
            if request:
                ...
                atomic_json(resume, {'document_kind': 'shallow_limit_ladder_resume_v1',
