#!/bin/bash
# c2p0 slot4: slot0 captions with every digit rewritten out.
#
# Same audio latents, recipe, seed and budget as every other c2p0 single-slot
# quarter. The only rows that move are the 8.6% of slot0 whose caption contained
# a digit (BPM, 4/4, Hz, years, …). Digit-free rows stay byte-identical.
...
EXP_PREFIX="phase8_qwen_caption2p0_slot4_no_digits_noq_${SCALE}"
EXPECTED_N=251599
LR=1e-4
BATCH=8
SEED=14159265
...
INPUTS="$HOME/exps_nvme/slot4/arm_inputs"
TRAIN_TSV="$INPUTS/phase8_caption2p0_slot4_train.tsv"
...
OVERLAY="$HOME/text_overlays/slot4"
...
log "[Step 1] rewrite slot0 digits -> slot4 tsv"
if [ ! -f "$TRAIN_TSV" ]; then
  "$PY" scripts/preprocess/rewrite_slot4_no_digits.py \
