#!/bin/bash
# c2p0 slot0-clean (definition A): slot0 captions with contamination regenerated.
#
# Same recipe as caption2p0_slot4v2_action.sh (055) except the corpus. Contaminated slot0
...
# Overlay reuses true_random via hardlink and re-encodes only the regenerated slot-0
# features, then trains with cap_index_fixed=0.
set -euo pipefail
...
EXP_PREFIX="phase8_qwen_caption2p0_slot0clean_defA_noq_${SCALE}"
LR=1e-4
BATCH=8
SEED=14159265

SRC_TSV="$DATA/phase8_qwen_caption10s_multisent_train.tsv"
INPUTS="$HOME/exps_nvme/slot0clean/arm_inputs"
TRAIN_TSV="$INPUTS/phase8_caption2p0_slot0clean_train.tsv"
...
# Overlay patch is a few hundred stacked npz (<1G) plus ~28G of checkpoints.
FREE_NVME=$(df -B1 --output=avail "$HOME" | tail -1)
if [ ! -f "$OVERLAY/DONE.json" ] && [ "$FREE_NVME" -lt 5000000000 ]; then
  log "[FAIL] NVMe free $((FREE_NVME/1000000000))G < 5G needed to patch overlay"
  exit 3
fi
...
# ---- Step 1/2: verify the PASS-gated inputs (built before queueing) --------
log "[Step 1] verify arm inputs"
[ -f "$MANIFEST" ] || { log "[FAIL] no manifest; inputs are built only after spot-check PASS"; exit 2; }
"$PY" - <<PYEOF
