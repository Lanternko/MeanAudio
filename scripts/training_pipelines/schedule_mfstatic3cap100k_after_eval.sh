...
LOG_DIR="$HOME/logs"
LOG="$LOG_DIR/mfstatic3cap100k_queue.log"
WAIT_SESSION="${WAIT_SESSION:-mfstatic3cap10k_shortdirect_eval}"
...
echo "$(ts) MF static-random-3cap 100k queue started"
echo "$(ts) [WAIT] session=$WAIT_SESSION, then GPU idle"
wait_for_tmux_session_done
wait_for_gpu_idle

cd "$WORK_DIR"
echo "$(ts) [START] MF-static-random-3cap 100k"
bash "$WORK_DIR/scripts/training_pipelines/train_pipeline_music_flamingo_static_random_3cap_100k.sh"
status=$?
echo "$(ts) [DONE] MF-static-random-3cap 100k status=$status"
exit "$status"
