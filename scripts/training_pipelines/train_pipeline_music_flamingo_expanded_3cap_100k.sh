...
EXPANDED_PREFIX="music_flamingo_slice10_100k_expanded_3cap"
...
python "$WORK_DIR/scripts/preprocess/prepare_music_flamingo_expanded_3cap.py" \
...
EXP_PREFIX="${EXP_PREFIX:-mfexpanded3cap100k_noq}" \
EXPECTED_N=300000 \
EXPECTED_CLIPS_N=100000 \
EXTRACT_MULTI_CAPTION=1 \
TRAIN_TSV="$DATA_DIR/${EXPANDED_PREFIX}_train.tsv" \
CLIPS_TSV="$DATA_DIR/${EXPANDED_PREFIX}_clips.tsv" \
JAMENDO_HOLDOUT_TSV="$DATA_DIR/${EXPANDED_PREFIX}_jamendo_holdout2048.tsv" \
NPZ_DIR="${NPZ_DIR:-/mnt/HDD/kojiek/phase4_jamendo_data/cache/${EXPANDED_PREFIX}_npz}" \
LATENT_DIR="${LATENT_DIR:-/mnt/HDD/kojiek/phase4_jamendo_data/cache/${EXPANDED_PREFIX}_latents_tmp}" \
NPZ_TSV="${NPZ_TSV:-/mnt/HDD/kojiek/phase4_jamendo_data/cache/${EXPANDED_PREFIX}_npz.tsv}" \
