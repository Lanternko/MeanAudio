#!/usr/bin/env bash
# Fast causal probe for the Phase 8 text-mask collapse.
#
# Start both arms from the same converged-but-low-CLAP masked S2 EMA, then:
#   1. evaluate it with Mask and legacy NoMask inference on the same 64 rows;
#   2. fine-tune identical copies for 2k steps on the same clean 128 rows;
#   3. evaluate both raw final weights on those same 64 rows.
#
# This is a repair/overfit probe. Its scores are not baseline benchmark scores.
...
BASE_WEIGHTS="$WORK_DIR/exps/phase8_bugfix_rerun_stage2_200000/phase8_bugfix_rerun_stage2_200000_ema_final.pth"
PREFIX=phase8_mask_repair_probe_n128
ITERATIONS=2000
SEED=42
...
echo "[Preflight] Verify fixed inputs and Stage 2 switching"
...
if (len(train_rows), len(cache), len(eval_rows)) != (128, 128, 64):
    raise SystemExit(
        f"bad subset sizes: train={len(train_rows)}, cache={len(cache)}, eval={len(eval_rows)}"
    )
for name in cache:
    with np.load(npz_dir / name) as data:
        if data["text_features"].shape != (77, 1024):
            raise SystemExit(f"bad clean NPZ: {name}")
print("fixed clean subset: train=128, eval=64; common initialization exists")
PY
...
train_arm() {
    local arm="$1"
    local use_mask="$2"
    local exp="${PREFIX}_${arm}_stage2_${ITERATIONS}"
    ...
    if [ "$curr_it" -eq "$ITERATIONS" ] && [ -f "$raw_weights" ]; then
        ...
        echo "[$arm] Stage 2 repair fine-tune: $curr_it -> $ITERATIONS (mask=$use_mask)"
        local init_arg=()
        ...
        torchrun --standalone --nproc_per_node=1 train.py \
            data=meanaudio model=meanaudio_s exp_id="$exp" \
...
labels = {
    ...
    "repair_nomask": f"{prefix}_nomask_stage2_{iterations}_train64",
    "repair_mask": f"{prefix}_mask_stage2_{iterations}_train64",
}
scores = {}
