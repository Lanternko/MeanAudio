...
PREFIX="lpmc_slice10_100k_control"

python "$WORK_DIR/scripts/preprocess/prepare_lpmc_slice10_control.py" \
    --ids-jsonl "$HOME/eval_output/music_flamingo_slice10_100k/caption.jsonl" \
    --lpmc-tsv "$DATA_DIR/_QUARANTINED_phase4_train.tsv" \
    --holdout-tsv "$DATA_DIR/phase4_test.tsv" \
    --out-dir "$DATA_DIR" \
    --n 100000 \
    --prefix "$PREFIX"

EXP_PREFIX="${EXP_PREFIX:-lpmc100k_noq}" \
EXPECTED_N=100000 \
