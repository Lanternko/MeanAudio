#!/bin/bash
# paired59k captioner-only control, full budget (S1 400k / S2 200k per arm).
#
...
EXP_PREFIX="paired59k_${ARM}_noq_full"
...
if [ "$FREE_NVME" -lt 60000000000 ]; then
  log "[FAIL] NVMe free $((FREE_NVME/1000000000))G < 60G needed for checkpoints"; exit 3
fi
