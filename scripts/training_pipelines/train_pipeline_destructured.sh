...
EXP_PREFIX="p_lpmc_destructured"        # EXP-A: test H10 (boilerplate template anchor)
                                        # P-LPMC-destructured: same as P8 baseline
                                        # (LP-MC NoQ single from phase7_v1_train.tsv)
                                        # but with caption boilerplate template
                                        # stripped via exp_a_v3_strip_phase7v1.py.
                                        # If MC CLAP drops from 0.185 toward Qwen-level
                                        # ~0.06 → boilerplate was the inductive anchor

BATCH_SIZE=8                      # 物理 batch size（每張 GPU）
ACCUM_STEPS=1                     # Gradient accumulation 步數
...
USE_Q_CONDITIONING=false          # NoQ baseline (matches P8)
...
COMMON_ARGS=(
    ...
    "data.AudioCaps_npz.tsv=$HOME/eval_tsvs_p100/phase7_v1_train_destructured.tsv"
    ...
    "++data.AudioCaps_npz.npz_dir=$HOME/exps_nvme/npz_phase7v1_destructured"
)
