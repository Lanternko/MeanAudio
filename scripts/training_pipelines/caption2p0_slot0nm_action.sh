#!/bin/bash
# c2p0 slot0nm: slot0clean (definition-A contamination regenerated) with every guessed
# measurement removed -- numbers (BPM, Hz, dB), musical key / mode / chord quality, and
# meter / time signature. Operator 2026-09-16: a captioning LM cannot measure these, so
# they are fabricated and must not reach training.
# Corpus: scripts/preprocess/rewrite_slot0nm_no_measurements.py (sentence-level, gated,
# measure-free sentences byte-identical) -> build_slot0nm_arm_inputs.py.
# Same recipe as 055 slot4v2 / 056 slot0clean except the corpus. Overlay reuses
# true_random via hardlink and re-encodes only changed slot-0 rows; cap_index_fixed=0.
set -euo pipefail
...
EXP_PREFIX="phase8_qwen_caption2p0_slot0nm_noq_${SCALE}"
LR=1e-4
BATCH=8
SEED=14159265
...
# Overlay patch is ~36k stacked npz (slot4v2 digits + key/meter + regenerated, ~32G) plus ~28G of checkpoints.
FREE_NVME=$(df -B1 --output=avail "$HOME" | tail -1)
if [ ! -f "$OVERLAY/DONE.json" ] && [ "$FREE_NVME" -lt 40000000000 ]; then
  log "[FAIL] NVMe free $((FREE_NVME/1000000000))G < 40G needed to patch overlay"
  exit 3
fi
...
# ---- Step 1/2: verify the built inputs -------------------------------------
log "[Step 1] verify arm inputs"
[ -f "$MANIFEST" ] || { log "[FAIL] no manifest; run build_slot0nm_arm_inputs.py first"; exit 2; }
"$PY" - <<PYEOF
...
names = [l.strip() for l in open("$CACHE_LIST") if l.strip()]
...
df = pd.read_csv("$TRAIN_TSV", sep="\t").to_dict("records")
...
print(f"  rows={len(rows)} changed_vs_source={m['changed_vs_source']} excluded={len(m['excluded'])}",
      "methods", json.dumps(m["rewrite_summary"]["sentence_methods"]))
PYEOF
