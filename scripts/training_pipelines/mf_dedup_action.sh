#!/bin/bash
# Music Flamingo full coverage, duplicate captions repaired.
#
# Same corpus, same audio latents, same recipe and budget as mf_fullcov. The
# only thing that moved is the 27,264 clips whose caption was an exact
# duplicate of another clip's: those were regenerated against a longer, hotter
# sampling ladder (--max-attempts 8, temperature 0.7 + 0.2*attempt) with the
# uniqueness set seeded from the 224,335 captions that were already unique.
# Everything else about the captioner call -- prompt short_direct_v2, 77-token
# window, max-new-tokens 80, batch 16, seed 4242 -- is byte-identical, so the
# repaired corpus stays homogeneous.
#
# Why this arm exists: mf_fullcov quarter landed at CFG0 CLAP 0.1892 against a
# 0.1900 gate while carrying a 0.8916 unique-rate deviation. Those two facts
# are entangled -- a corpus where 13.9% of rows share a caption with another
# clip is a weaker conditioning signal, and the gate miss is 0.0008. This
# separates them: if the repaired corpus scores materially higher, the deficit
# was duplication; if it does not, duplication was not the binding constraint
# and the remaining gap belongs to the captioner.
#
# The winner of {mf_fullcov, mf_dedup} at quarter is the one that earns the
# full budget. See docs/experiments/mf_full_coverage_line.md.
set -eo pipefail
...
EXP_PREFIX="mf_dedup_noq_${SCALE}"
EXPECTED_N=251599
LR=1e-4
BATCH=8
SEED=14159265

INPUTS="$HOME/exps_nvme/mf_dedup/arm_inputs"
TRAIN_TSV="$INPUTS/mf_dedup_train.tsv"
CACHE_LIST="$DATA/phase8_qwen_official_matched_npz_cache_train.txt"
NPZ_DIR="/mnt/HDD/kojiek/phase8_qwen_official_matched_npz"
OVERLAY="$HOME/text_overlays/mf_dedup"
JSONL="$HOME/eval_output/mf_recaption_dedup/caption.jsonl"
...
OVERRIDE="$HOME/exps_nvme/mf_dedup/PROCEED_TO_FULL_ANYWAY"
...
if [ "$SCALE" = "full" ] && [ ! -f "$OVERRIDE" ]; then
  "$PY" "$DECIDE" --assert-winner mf_dedup \
...
if [ ! -f "$INPUTS/bindings.json" ] || [ ! -f "$TRAIN_TSV" ]; then
  "$PY" scripts/preprocess/build_mf_fullcov_arm_inputs.py \
    --jsonl "$JSONL" --out-dir "$INPUTS" --out-name mf_dedup_train.tsv 2>&1 | tee "$STATE/build_inputs.log"
else
...
# The full-coverage corpus needed a 0.89 deviation to launch (0.8916 unique).
# This corpus exists to remove that deviation, so the bar is back at CLAUDE.md's
# original 0.90 with no exception. If the repair pass did not clear it, the run
# must not start -- a second deviation would make the line unreadable.
