"""Contract-bound per-caption negative evaluator; isolated from queued eval.py users."""
import sys
...
def main():
    ...
    parser.add_argument('--negative_prompt_column', required=True)
    ...
    with open(eval_file, 'r') as f:
            ...
            for row in reader:
                negative = row[args.negative_prompt_column]
                if not negative.strip():
                    raise ValueError('empty per-caption negative')
                row_negatives.append(negative)
    ...
    for k in tqdm(range(0, len(text_prompts))):
        negative_prompt = row_negatives[k]
        negative_text_arg = [negative_prompt]
