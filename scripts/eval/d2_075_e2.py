...
def cell(arm, c, lvl, seed=14159265):
    name = f'phase8_qwen_caption2p0_slot0clean_{ARMS[arm]}_noq_quarter_s{seed}_mc_mf25_{c}'
...
def main():
    ...
    ap.add_argument('--seed', type=int, default=14159265, help='training seed of all three arms')
    a = ap.parse_args()
    rng = np.random.default_rng(20260923)
    D = {arm: {c: cell(arm, c, a.lvl, a.seed) for c in ['cfg0', 'cfg3_neg']} for arm in ARMS}
    ids = sorted(set.intersection(*[set(D[arm][c]) for arm in ARMS for c in D[arm]]))
    print(f'# 075 E2/E3 ({"level-matched -30 LUFS" if a.lvl else "raw"}), train seed {a.seed}, n={len(ids)} paired clips\n')
