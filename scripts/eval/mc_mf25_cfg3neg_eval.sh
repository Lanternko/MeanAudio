...
#
# Usage: mc_mf25_cfg3neg_eval.sh <exp_name> <s2_ema.pth>
...
log(){ echo "[$(date -u +%FT%TZ)] [cfg3neg $EXP] $*"; }
...
if [ ! -f "$OUT/metrics.txt" ] && [ ! -f "$OUT/$LABEL/metrics.txt" ]; then
  ...
  if [ "$HAVE" -lt 5400 ]; then
    ...
    "$PY" eval.py --variant meanaudio_s --model_path "$CKPT" \
      ...
      --seed 42 --full_precision --no_q 2>&1 | tee "$LOGDIR/gen.log"
  fi
...
"$PY" - "$REPORT" "$METRICS" "$CKPT" "$MC_TSV" "$LABEL" "$NEG" <<'PY'
import hashlib, json, math, os, sys
from datetime import datetime, timezone
from pathlib import Path

report, metrics, ckpt, tsv = map(Path, sys.argv[1:5])
label, neg = sys.argv[5:7]
