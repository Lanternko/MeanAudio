#!/usr/bin/env python3
"""061 preflight: immutable identity, provenance and capacity checks before any GPU work."""
...
from crest_intervention_cfg3_20260916 import CONTRACT, arms, binding, config, digest  # noqa: E402
...
def main() -> int:
    ...
    for key in ('source_manifest', 'source_audio_root', 'tsv', 'aes_snapshot'):
    ...
    names = list(arms(c))
    if c['analysis']['reference_arm'] not in names or c['analysis']['primary_arm'] not in names:
        problems.append('analysis arms not declared in transform.arms')
    for name, spec in arms(c).items():
        if spec['kind'] == 'shifted_envelope' and spec['source_arm'] not in names:
            problems.append(f'{name}: source arm {spec["source_arm"]} not declared')
    if c['protocol']['aes_conditions'] != c['protocol']['rows'] * len(names):
        problems.append('declared AES condition count does not match rows x arms')
    ...
    print(f'PASS: contract {binding()[:12]} / {c["protocol"]["rows"]} rows x {len(names)} arms '
          f'({", ".join(names)}) / baseline provenance sampled', flush=True)
    return 0
