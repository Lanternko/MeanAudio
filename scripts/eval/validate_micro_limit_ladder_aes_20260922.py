#!/usr/bin/env python3
"""074 preflight: identity, provenance and capacity checks before any GPU work.
...
from micro_limit_ladder_aes_20260922 import (CROSS_CHECK_RUNG_DB, LADDER_DB,  # noqa: E402
                                             LIMIT_DB, SHALLOW072, configure)
...
def main() -> int:
    ...
    # 063 supplies the replication gates inside analyze(), 072 the D3 cross-check.
    for name, root in (('063 gain ladder items', Path(cfg['gain_ladder_063_items'])),
                       ('072 shallow ladder items', Path(SHALLOW072) / 'items')):
        if not root.is_dir():
    ...
    if not problems:
        ...
        # 072 must actually hold the rung this run is gated against.
        missing = [i for i in sorted(ids)[:1]
                   if not (Path(SHALLOW072) / 'items' / (i + '.json')).is_file()]
        if missing:
            problems.append(f'072 items incomplete: {missing[0]}')
        elif CROSS_CHECK_RUNG_DB not in json.loads(
                (Path(SHALLOW072) / 'summary.json').read_text())['config']['limit_db']:
            problems.append(f'072 did not measure D{CROSS_CHECK_RUNG_DB}; no cross-check possible')
    ...
    print(f'PASS: contract {digest(contract_path)[:12]} / {cfg["rows"]} rows / '
          ...
          f'072 D{CROSS_CHECK_RUNG_DB} available', flush=True)
    return 0
