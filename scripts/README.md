...
**Canonical eval: `mc_mf25_eval.sh <exp> <ema.pth> (--no_q | --quality_level N)`**（2026-09-18 起）— MusicCaps 5521 / MF25 / seed 42 / NoMask，兩格 CFG0 + CFG3+neg（fidelity8），metrics 走 `eval_metrics.py`；有 REPORT 跳過、殘缺目錄整格重跑（eval.py 單一 RNG，補齊不具決定性）。生成與舊 CFG0 harness / `mc_mf25_cfg3neg_eval*.sh` 逐樣本一致。`train_pipeline.sh` 已改用它。
